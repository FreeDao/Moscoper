...
public class KeywordsFlow extends FrameLayout implements OnGlobalLayoutListener {
    ...
    private OnItemEmptyListener itemEmptyListener;
    ...
    public boolean hasTag(String tag){
    	if(StringUtil.getInstance().IsEmpty(tag)) return false;
    	for (int i = 0; i < vecKeywords.size(); i++) {
			Object obj = vecKeywords.get(i);
			String tagName = null;
			...
			if(null != tagName && tagName.equalsIgnoreCase(tag)) return true;
		}
    	return false;
    }
    ...
    public boolean feedKeyword(Object keyword) {  
    ...
    public void checkEmpty(){
    	if(vecKeywords.size() == 0 && null != itemEmptyListener){
    		itemEmptyListener.onEmpty();
    	}else if(vecKeywords.size() > 0 && null != itemEmptyListener){
    		itemEmptyListener.onNotEmpty();
    	}
    }
        ...
        if (width > 0 && height > 0 && vecKeywords != null && vecKeywords.size() > 0 && enableShow) {  
            ...
            for (int i = 0; i < size; i++) {  
                ...
                if(obj instanceof CategoryRec){
                	...
                	keyword = obj.toString();
                	Trace.e("keywordsFlow", "object has other type:"+keyword);
                }
                // 随机颜色   
                ...
                txt.setOnLongClickListener(itemLongClickListener);
                txt.setText(keyword);  
                ...
                txt.setTag(R.id.cloud_tag_first, xy);txt.setTag(R.id.cloud_tag_second, obj);  
                if (xy[IDX_Y] > yCenter) {  
    ...
    private void attach2Screen(LinkedList<TextView> listTxt, int xCenter, int yCenter, int yItem) {  
        ...
        for (int i = 0; i < size; i++) {  
            ...
            int[] iXY = (int[]) txt.getTag(R.id.cloud_tag_first);  
            ...
            inner: for (int k = i - 1; k >= 0; k--) {  
                int[] kXY = (int[]) listTxt.get(k).getTag(R.id.cloud_tag_first);  
    ...
    private void sortXYList(LinkedList<TextView> listTxt, int endIdx) {  
        for (int i = 0; i < endIdx; i++) {  
            for (int k = i + 1; k < endIdx; k++) {  
                if (((int[]) listTxt.get(k).getTag(R.id.cloud_tag_first))[IDX_DIS_Y] < ((int[]) listTxt.get(i).getTag(R.id.cloud_tag_first))[IDX_DIS_Y]) {  
    ...
    public void setOnItemEmptyListener(OnItemEmptyListener listener) {  
        itemEmptyListener = listener; 
    }
    ...
    public boolean removeTag(Object tag){
    	...
    	boolean result = vecKeywords.remove(tag);
	...
	public void removeAllTags() {
		if(null != vecKeywords) vecKeywords.clear();
		go2Show(KeywordsFlow.ANIMATION_IN);
    	checkEmpty();
	}
}
