 ...
 * 
 */
public class TagReviewListActivity extends BaseActivity {
	private static final String TAG = "TagReviewListActivity";

	/**
	 * 传递值为CategoryRec类型，标题为其属性Name
	 */
	public static final String KEY_TAG_REC = "key_tag_rec";
	...
	private void chanceMyFvaDialog(final ProductRec rec, final int pos) {
		final MyRemDialog chanceMyFvaDialog = new MyRemDialog(
				TagReviewListActivity.this, R.style.dialog);
	...
	private void Init() {
		...
		if (null != mIntent && mIntent.hasExtra(KEY_TAG_REC)) {
			tagRec = (TagRec) mIntent
					.getSerializableExtra(KEY_TAG_REC);
		}
	}
	...
	private void InitTopBar() {
		...
		if (null != tagRec) {
			tvTitle.setText(tagRec.getName());
		}
	}
...
//						if(!isNeedRemoveProduct && null != catId && null != currPageCatId && !catId.equalsIgnoreCase(currPageCatId)){
//							isNeedRemoveProduct = true;
//						}
//					}
				}
...
//					productList.get(pos).setFavNum(productRec.getFavNum());
	...
	private void readyToGetAllGoodsByTagID(final String Id,
			...
			public void onFailure(Throwable t, String strMsg) {
				...
				if (BuildConfig.isDebug) {
					...
					ExceptionRemHelper.showExceptionReport(
							TagReviewListActivity.this, msg);
				} else {
					Toast.makeText(TagReviewListActivity.this,
