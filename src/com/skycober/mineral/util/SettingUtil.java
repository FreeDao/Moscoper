...
import com.skycober.mineral.network.NetUtils;
import com.skycober.mineral.network.NetUtils.Callback;
...
public class SettingUtil {
	...
	public static final String appKey = "pvxdm17jx5lmr";
	public static final String appSecret = "rndKi5V6nGvMU2";

	public void saveUserInfo(String userId, final UserRec mRec) {
		String rootKey = SETTING_USER_PREF + userId;

		final SharedPreferences pref = mContext.getSharedPreferences(rootKey,
				Context.MODE_PRIVATE);

		final Editor editor = pref.edit();
		NetUtils.getUserToken(userId, mRec.getUserName(), mRec.getAvatar()
				.getNormal(), appKey, appSecret, new Callback() {
			...
			public void onCallBack(String token) {
				...
				RongIM.connect(token, new RongIMClient.ConnectCallback() {
					...
					public void onSuccess(String arg0) {
						// TODO Auto-generated method stub
						System.out.println("====RongIM====onSuccess===");
					}
				...
				editor.putString(AvatarRec.ResponseSmall, null == mRec
						.getAvatar() ? null : mRec.getAvatar().getSmall());
				editor.putString(AvatarRec.ResponseNormal, null == mRec
						.getAvatar() ? null : mRec.getAvatar().getNormal());
				editor.putString(AvatarRec.ResponseBig, null == mRec
						.getAvatar() ? null : mRec.getAvatar().getBig());
				...
				editor.putString(UserRec.ResponseOfficePhone,
						mRec.getOfficePhone());
				editor.putString(UserRec.ResponseMobilePhone,
						mRec.getMobilePhone());
	...
	public UserRec getUserInfo(String userId, UserRec defaultValue) {
		...
		if (null == pref
				|| StringUtil.getInstance().IsEmpty(
						pref.getString(UserRec.ResponseUserId, "")))
			return defaultValue;
		...
		long regTime = pref.getLong(UserRec.ResponseRegTime, mRec.getRegTime());
		mRec.setRegTime(regTime);
		String QQ = pref.getString(UserRec.ResponseQQ, mRec.getQq());
		mRec.setQq(QQ);
		String birthday = pref.getString(UserRec.ResponseBirthday,
				mRec.getBirthday());
		mRec.setBirthday(birthday);
		String msn = pref.getString(UserRec.ResponseMSN, mRec.getMsn());
		mRec.setMsn(msn);
		String homePhone = pref.getString(UserRec.ResponseHomePhone,
				mRec.getHomePhone());
		mRec.setHomePhone(homePhone);
		String officePhone = pref.getString(UserRec.ResponseOfficePhone,
				mRec.getOfficePhone());
		mRec.setOfficePhone(officePhone);
		String mobilePhone = pref.getString(UserRec.ResponseMobilePhone,
				mRec.getMobilePhone());
		mRec.setMobilePhone(mobilePhone);
		long visitCount = pref.getLong(UserRec.ResponseVisitCount,
				mRec.getVisitCount());
		mRec.setVisitCount(visitCount);
		return mRec;
	}
	...
	public void clearLoginInfo() {
		this.saveValue(SettingUtil.KEY_LOGIN_USER_ID,
